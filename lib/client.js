...
Client.prototype.consume = function (/*exchange, bindings, callback*/) {
	var self = this, callback, opts = { },
		args = util.convertArgs(arguments);
	
	if (typeof args[0] === 'object' && args.length === 3) {
		opts = args.shift();
...
Client.prototype.send = function (routingKey, content, headers) {
	var res = this.defaultChan.publish(routingKey, msgpack.pack(content), headers);
};
