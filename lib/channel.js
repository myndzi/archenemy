...
Channel.prototype.init = function (chan, opts) {
	...
	.then(function () {
		...
		Object.keys(opts.bindings).forEach(function (queue) {
			...
			queues.push(
				chan.assertQueue(queue, opts.queueOpts).then(function (res) {
					var queue = res.queue;
					self.log.trace('('+self.id +') assertQueue succeeded: '+ queue);
					self.queues.push(queue);

					return when.all(
						patterns.map(function (pattern) {
							self.log.trace([opts.exchange, pattern, queue].join(' -> '));
