	...
	self.consume = util.queuedProxy(function (/*queues, callback, errback*/) {
		...
		return when.all(queues.map(function (queue) {
			return self.chan.consume(queue, function (msg) {
				var parsed = msgpack.unpack(msg.content),
					deferred = when.defer();
				
				var ack = function () {
					if (self.connection.closing) {
						self.log.warn('Can\'t ack: connection is closing');
					} else {
						self.chan.ack(msg);
					}
				};
				...
				var stack = msg.properties.headers.stack,
					headers = msg.properties.headers;
				var next = function (res) {
					self.log.trace('Stack: ', stack);
					if (stack.length) {
						var next = stack.shift();
						next.headers.stack = stack;
						
						self.log.trace('Message continuation: next up: ', next.routingKey);
						
						self.publish(
							next.routingKey,
							msgpack.pack(res),
							extend(headers, next.headers)
						);
					}
				}
				...
				when.fn.call(function () {
					...
					if (when.isPromise(res)) {
						res.then(function (res) {
							self.log.trace('returned promise result: ', res);
							next(res);
							return res;
						});
					} else if (typeof res !== 'undefined') {
						deferred.promise.then(function (res) {
							self.log.trace('deferred promise result: ', res);
							next(res);
							return res;
						});
